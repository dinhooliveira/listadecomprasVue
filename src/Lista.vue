  ...
  <div class="container mt-4 mb-4">
    <div class="col-md-12 text-right">
      <a
        ...
        :class="listas.length < 1 ? 'pulse' : ''"
      >
        <span class="text-white" v-if="listas.length < 1">
          Incluir uma nova lista
        </span>
      ...
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          <div class="modal-header bg-success text-white">
            ...
            <h5
              class="text-center"
              v-if="this.acaoEditar && this.lista.id !== ''"
            >
              Editar Lista Código ({{ this.lista.id }})
            </h5>
          </div>
        ...
        <tbody>
          <tr v-for="(lista, index) in listas" v-bind:key="lista.id">
            ...
            <td style="width: 50px">
              <div class="row p-1">
                <a
                  @click="editarLista(lista.id)"
                  class="btn btn-sm btn-primary text-white m-1"
                >
                  <img width="15px" src="./assets/icon/edit.png" />
                </a>
                <a
                  @click="removeLista(lista.id)"
                  class="btn btn-sm btn-danger text-white m-1"
                >
                  <img width="15px" src="./assets/icon/remove.png" />
                </a>
                <a
                  @click="whatsappShared(lista.id)"
                  class="btn btn-sm btn-success text-white ml-1"
                >
                  <img width="15px" src="./assets/icon/whatsapp.png" />
                </a>
                <div>
                  <a
                    @click="redirectItems(lista.id)"
                  ...
                  >
                    ...
                    <span v-if="totalLista(lista.id) > 0">{{
                      totalLista(lista.id)
                    }}</span>
                  </a>
                </div>
      ...
      >
        ...
        <div class="card-body">
          ...
          <div class="text-right">
            ...
            >
              <img width="30x" src="./assets/icon/edit.png"
            /></a>
            <a
              @click="removeLista(lista.id)"
            ...
            <a
              ...
              class="btn btn-sm btn-success text-white ml-1"
            >
              <img width="30x" src="./assets/icon/whatsapp.png" />
            </a>
            <a
              @click="redirectItems(lista.id)"
              class="btn btn-sm btn-info text-white ml-1"
              :class="totalLista(lista.id) < 1 ? 'pulse' : ''"
            >
...
export default {
  ...
  methods: {
    ...
    whatsappShared: function (id) {
      if (this.isMobile) {
        store.lista.whatsappMobileShared(id);
      } else {
        store.lista.whatsappWebShared(id);
      }
    },
  },
  beforeMount() {
